...
#weights = tf.get_variable(
#  'weights', [num_features, num_filters],
#  initializer=tf.variance_scaling_initializer(scale=1.0, mode='fan_in', distribution='normal', dtype=tf.float32),
#  dtype=tf.float32
#)
...
#features_flat = tf.reshape(features, [2*half_batch*max_steps, num_features])
#filters_flat = tf.matmul(features_flat, weights)+bias
#filters = tf.reshape(filters_flat, [2*half_batch, max_steps, num_filters])

# Aggregate filters
#
scores = tf.reduce_max(filters, axis=2)

# Aggregate kmers
#
logits = tf.reduce_max(scores+penalties[:,step_size-1:], axis=1)
